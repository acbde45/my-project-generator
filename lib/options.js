...
const rcPath = (exports.rcPath = getRcPath('.mpgrc'));
...
const schema = createSchema((joi) =>
  joi.object().keys({
    ...
    packageManager: joi.string().valid('yarn', 'npm', 'pnpm', 'tnpm'),
...
exports.loadOptions = () => {
  ...
  if (fs.existsSync(rcPath)) {
    try {
      ...
      error(
        `Error loading saved preferences: ` +
          `~/.mpgrc may be corrupted or have syntax errors. ` +
          `Please fix/delete it and re-run mpg in manual mode.\n` +
          `(${e.message})`,
      );
      exit(1);
    }
    validate(cachedOptions, schema, () => {
      error(
        `~/.mpgrc may be outdated. ` +
          `Please delete it and re-run mpg in manual mode.`,
      );
    });
    return cachedOptions;
  } else {
